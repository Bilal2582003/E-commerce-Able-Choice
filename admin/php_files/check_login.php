...
if (isset($_POST['login'])) {
    ...
    if (!isset($_POST['name']) || empty($_POST['name'])) {
        echo json_encode(array('error' => 'name_empty'));
        exit;
        ...
        if (!empty($result)) {
            ...
            $theme = $db->select("options");
            $res = $db->getResult();
            $db->select("theme", "*", null, 'id = ' .$res[0]['theme_id']);
            $res = $db->getResult();
            if (count($res) > 0) {
            ...
            echo json_encode(array('error' => 'flase'));
            exit;
        }
    }
}
...
if (isset($_POST['changePass'])) {
    if (!isset($_POST['old_pass']) || empty($_POST['old_pass'])) {
        echo json_encode(array('error' => 'Old password is empty.'));
        exit;
    } else if (!isset($_POST['new_pass']) || empty($_POST['old_pass'])) {
        echo json_encode(array('error' => 'New password is empty.'));
        exit;
    } else {
