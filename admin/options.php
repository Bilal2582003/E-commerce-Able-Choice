            ...
            <form id="updateOptions" class="add-post-form row" method="post" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="contact_address" class="form-control address" cols="30" rows="2"><?php echo $row['contact_address']; ?></textarea>
                    </div>
                    <div class="form-group">
                        <label>Theme</label>
                        <select name="theme" class="form-control theme">
                            ...
                            <?php
                        $db->select('theme','*',null,null,null,null);
                        $resultT = $db->getResult();
                        if ($resultT > 0) {
                            foreach($resultT as $rowT) {
                                $select = $rowT['id'] == $row['theme_id'] ? 'selected' : '';
                                echo '<option '.$select.' value="'.$rowT['id'].'" >'.$rowT['color'].'</option>';
                            }
                        }
                        ?>
                        </select>
